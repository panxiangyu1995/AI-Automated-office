## 1. Preparation
- [ ] 1.1 Confirm dependency stories are complete
- [ ] 1.2 Confirm FR, NFR, ARCH, and UX mapping
- [ ] 1.3 Confirm this story still matches the agent-runtime-rebaseline sequence

## 2. Implementation
- [ ] 2.1 Connect retry and replan decisions to real execution outcomes
- [ ] 2.2 Implement checkpoint save, activate, rollback, and restore
- [ ] 2.3 Trigger recovery from tool failure, timeout, and interruption
- [ ] 2.4 Feed recovery state back to chat and debug views
- [ ] 2.5 Verify interruption, retry, rollback, and restore end to end

## 3. Verification
- [ ] 3.1 Unit and integration tests updated
- [ ] 3.2 Lint and build pass
- [ ] 3.3 Acceptance criteria verified against real runtime behavior

## 4. Documentation
- [ ] 4.1 Update progress.txt
- [ ] 4.2 Update task.json passes when done