...
### Requirement: Story 4.12 - Context Compression
The system SHALL implement the full acceptance scope for Story 4.12 while staying aligned with FR(FR-CTX-1, FR-CTX-2, FR-CTX-3, FR-CTX-4, FR-CTX-5), NFR(NFR8-1), ARCH(ADR-043, ADR-044), and UX(UX-01, UX-04).

#### Scenario-1: Acceptance item 1
- **GIVEN** prerequisite dependencies are complete and the user has valid permission context
- **WHEN** Detect token threshold crossing in active sessions
- **THEN** runtime state, persisted records, and UI status stay consistent and auditable

#### Scenario-2: Acceptance item 2
- **GIVEN** prerequisite dependencies are complete and the user has valid permission context
- **WHEN** Trigger structured summary generation and recent-history retention
- **THEN** runtime state, persisted records, and UI status stay consistent and auditable

#### Scenario-3: Acceptance item 3
- **GIVEN** prerequisite dependencies are complete and the user has valid permission context
- **WHEN** Store compression output into session memory
- **THEN** runtime state, persisted records, and UI status stay consistent and auditable
