## 1. Preparation
- [ ] 1.1 Confirm dependency stories are complete
- [ ] 1.2 Confirm FR, NFR, ARCH, and UX mapping
- [ ] 1.3 Confirm this story still matches the agent-runtime-rebaseline sequence

## 2. Implementation
- [ ] 2.1 Define provider traits and request-response contracts
- [ ] 2.2 Implement PromptBuilder with system prompt, runtime context, and tool visibility
- [ ] 2.3 Support provider selection, timeout, retry, and error mapping
- [ ] 2.4 Connect PromptBuilder to AgentOrchestrator
- [ ] 2.5 Persist model outputs through the runtime pipeline

## 3. Verification
- [ ] 3.1 Unit and integration tests updated
- [ ] 3.2 Lint and build pass
- [ ] 3.3 Acceptance criteria verified against real runtime behavior

## 4. Documentation
- [ ] 4.1 Update progress.txt
- [ ] 4.2 Update task.json passes when done