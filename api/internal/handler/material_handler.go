...
func NewMaterialHandler(matService *service.MaterialService) *MaterialHandler {
	return &MaterialHandler{matService: matService}
}
...
func (h *MaterialHandler) Update(c *gin.Context) {
	...
	matID := c.Param("id")
	if matID == "" {
		response.ValidationError(c, "id", "物料ID不能为空")
		return
	}
