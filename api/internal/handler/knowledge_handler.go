...
func (h *KnowledgeHandler) SemanticSearch(c *gin.Context) {
	...
	query := c.Query("q")
	if query == "" {
		response.ValidationError(c, "q", "搜索关键词不能为空")
		return
	}
...
func (h *KnowledgeHandler) ChunkDocument(c *gin.Context) {
	...
	docID := c.Param("id")
	if docID == "" {
		response.ValidationError(c, "id", "文档ID不能为空")
		return
	}
...
func (h *KnowledgeHandler) ListVersions(c *gin.Context) {
	...
	if err != nil {
		response.Error(c, &errors.AppError{Code: "KB_VERSION_ERROR", Message: err.Error(), Status: 500})
		return
	}
...
func (h *KnowledgeHandler) GetVersion(c *gin.Context) {
	...
	if err != nil {
		response.ValidationError(c, "version", "版本号无效")
		return
	}
...
func (h *KnowledgeHandler) CompareVersions(c *gin.Context) {
	...
	if err1 != nil || err2 != nil {
		response.ValidationError(c, "v1/v2", "版本号无效")
		return
	}
