...
func (s *MaterialService) Create(enterpriseID, name, skuCode, materialType, spec, unit string, unitPrice float64) (*model.Material, *apperrors.AppError) {
	...
	if name == "" { return nil, apperrors.NewValidationError("name", "物料名称不能为空") }
	...
	m.EnterpriseID = eid
	if err := s.matRepo.Create(m); err != nil { return nil, apperrors.ErrInternal.WithDetail("创建物料失败: " + err.Error()) }
	return m, nil
}
...
func (s *MaterialService) Update(matID, name, materialType, spec, unit string, unitPrice float64, status string) (*model.Material, *apperrors.AppError) {
	...
	if name != "" { m.Name = name }
	if materialType != "" { m.MaterialType = materialType }
	if spec != "" { m.Spec = spec }
	if unit != "" { m.Unit = unit }
	if unitPrice > 0 { m.UnitPrice = unitPrice }
	if status != "" { m.Status = status }
	if err := s.matRepo.Update(m); err != nil { return nil, apperrors.ErrInternal.WithDetail("更新物料失败: " + err.Error()) }
	return m, nil
}
...
func (s *MaterialService) List(enterpriseID string, page, pageSize int) ([]model.Material, int64, *apperrors.AppError) {
	eid, err := uuid.Parse(enterpriseID)
	if err != nil { return nil, 0, apperrors.NewValidationError("enterprise_id", "企业ID无效") }
	ms, total, err := s.matRepo.ListByEnterprise(eid, page, pageSize)
	if err != nil { return nil, 0, apperrors.ErrInternal.WithDetail("查询物料列表失败: " + err.Error()) }
	return ms, total, nil
}
