...
func (s *WarehouseService) Update(wID, name, code, addr, status string) (*model.Warehouse, *apperrors.AppError) {
	id, err := uuid.Parse(wID); if err != nil { return nil, apperrors.NewValidationError("warehouse_id", "仓库ID无效") }
	w, err := s.repo.FindByID(id); if err != nil { return nil, apperrors.ErrInternal.WithDetail("查询仓库失败") }
...
func (s *WarehouseService) Delete(wID string) *apperrors.AppError {
	id, err := uuid.Parse(wID); if err != nil { return apperrors.NewValidationError("warehouse_id", "仓库ID无效") }
	w, err := s.repo.FindByID(id); if err != nil { return apperrors.ErrInternal.WithDetail("查询仓库失败") }
	if w == nil { return apperrors.ErrNotFound.WithDetail("仓库不存在") }
	if err := s.repo.Delete(id); err != nil { return apperrors.ErrInternal.WithDetail("删除仓库失败: "+err.Error()) }
	return nil
}

func (s *WarehouseService) Get(wID string) (*model.Warehouse, *apperrors.AppError) {
	id, err := uuid.Parse(wID); if err != nil { return nil, apperrors.NewValidationError("warehouse_id", "仓库ID无效") }
	w, err := s.repo.FindByID(id); if err != nil { return nil, apperrors.ErrInternal.WithDetail("查询仓库失败") }
	if w == nil { return nil, apperrors.ErrNotFound.WithDetail("仓库不存在") }
	return w, nil
