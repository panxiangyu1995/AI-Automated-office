...
func (s *KnowledgeService) CreateFile(eid, name, path, ftype, category, refID, refType string, size int64) (*model.FileRecord, *apperrors.AppError) {
	...
	r.EnterpriseID = id
	if err := s.repo.CreateFile(r); err != nil {
		return nil, apperrors.ErrInternal.WithDetail("创建文件记录失败")
	}
	return r, nil
}
...
func (s *KnowledgeService) CreateDoc(eid, title, categoryID, content, summary, tags string) (*model.KnowledgeDoc, *apperrors.AppError) {
	...
	d.EnterpriseID = id
	if err := s.repo.CreateDoc(d); err != nil {
		return nil, apperrors.ErrInternal.WithDetail("创建文档失败")
	}
	return d, nil
}
...
func (s *KnowledgeService) CreateCategory(eid, name, parentID string) (*model.KBCategory, *apperrors.AppError) {
	...
	if err := s.repo.CreateCategory(c); err != nil {
		return nil, apperrors.ErrInternal.WithDetail("创建分类失败")
	}
	return c, nil
}
...
func (s *KnowledgeService) ChunkDocument(enterpriseID, docID string) ([]model.DocChunk, *apperrors.AppError) {
	...
	if dbErr != nil {
		return nil, apperrors.ErrInternal.WithDetail("查询文档失败")
	}
	if doc == nil {
...
func (s *KnowledgeService) keywordSearch(eidUUID uuid.UUID, query string, limit int) ([]map[string]interface{}, *apperrors.AppError) {
	...
	if dbErr != nil {
		return nil, apperrors.ErrInternal.WithDetail("关键词搜索失败")
	}
