...
func NewOperationsService(repo repository.OperationsRepository) *OperationsService {
...
func (s *OperationsService) CreatePlan(eid, name, desc, features string, price float64, maxUsers int, maxStorage int64) (*model.SubscriptionPlan, *apperrors.AppError) {
	...
	if err := s.repo.CreatePlan(p); err != nil {
		return nil, apperrors.ErrInternal.WithDetail("创建套餐失败")
	}
	return p, nil
}
...
func (s *OperationsService) CreateSkill(eid, name, desc, params, endpoint, module string) (*model.Skill, *apperrors.AppError) {
	...
	if err := s.repo.CreateSkill(sk); err != nil {
		return nil, apperrors.ErrInternal.WithDetail("创建Skill失败")
	}
	return sk, nil
}
...
func (s *OperationsService) CreateWebhook(eid, name, url, secret, events string) (*model.Webhook, *apperrors.AppError) {
	...
	if err := s.repo.CreateWebhook(w); err != nil {
		return nil, apperrors.ErrInternal.WithDetail("创建Webhook失败")
	}
	return w, nil
}
