...
func (s *ContractService) Create(eid, customerID, name, content, notes string, amount float64) (*model.Contract, *apperrors.AppError) {
	...
	if name == "" {
		return nil, apperrors.NewValidationError("name", "合同名称不能为空")
	}
	c := &model.Contract{
	...
	if err := s.repo.Create(c); err != nil {
		return nil, apperrors.ErrInternal.WithDetail("创建合同失败: " + err.Error())
	}
	return c, nil
}
...
func (s *ContractService) PatchFields(eid, cID string, fields map[string]interface{}) (*model.Contract, *apperrors.AppError) {
	...
	if err != nil {
		return nil, apperrors.NewValidationError("contract_id", "无效")
	}
	entID, _ := uuid.Parse(eid)
	c, dbErr := s.repo.FindByID(id, entID)
	if dbErr != nil {
		return nil, apperrors.ErrInternal.WithDetail("查询合同失败")
	}
	if c == nil {
		return nil, apperrors.ErrNotFound.WithDetail("合同不存在")
	}
	...
	if len(updates) == 0 {
		return nil, apperrors.NewValidationError("fields", "没有可更新的字段")
	}

	result, dbErr := s.repo.PatchFields(id, c.EnterpriseID, updates)
	if dbErr != nil {
		return nil, apperrors.ErrInternal.WithDetail("更新合同字段失败: " + dbErr.Error())
	}
	return result, nil
}
...
func (s *ContractService) Update(eid, cID, name, content, notes string, amount float64) (*model.Contract, *apperrors.AppError) {
	...
	if c.Status != "draft" {
		return nil, apperrors.ErrBadRequest.WithDetail("仅草稿状态可编辑")
	}
	if name != "" {
		c.Name = name
	}
	...
	if err := s.repo.Update(c); err != nil {
		return nil, apperrors.ErrInternal.WithDetail("更新合同失败: " + err.Error())
	}
	return c, nil
}
...
func (s *ContractService) Delete(eid, cID string) *apperrors.AppError {
	id, err := uuid.Parse(cID)
	if err != nil {
		return apperrors.NewValidationError("contract_id", "无效")
	}
	entID, _ := uuid.Parse(eid)
	c, dbErr := s.repo.FindByID(id, entID)
	if dbErr != nil {
		return apperrors.ErrInternal.WithDetail("查询合同失败")
	}
	if c == nil {
		return apperrors.ErrNotFound.WithDetail("合同不存在")
	}
	if c.Status != "draft" {
		return apperrors.ErrBadRequest.WithDetail("仅草稿状态可删除")
	}
	if err := s.repo.Delete(c, c.EnterpriseID); err != nil {
		return apperrors.ErrInternal.WithDetail("删除合同失败: " + err.Error())
	}
	return nil
}
...
func (s *ContractService) ChangeStatus(eid, cID, newStatus string) (*model.Contract, *apperrors.AppError) {
	...
	if newStatus == "fulfilled" || newStatus == "terminated" {
		now := time.Now()
		c.ExpireAt = &now
	}
	if err := s.repo.Update(c); err != nil {
...
func (s *ContractService) SaveAttachment(eid, cID, fileName, contentType string, fileSize int64, data io.Reader) (*model.ContractAttachment, *apperrors.AppError) {
	...
	if err := s.repo.CreateAttachment(att); err != nil {
		return nil, apperrors.ErrInternal.WithDetail("保存附件记录失败")
	}
	return att, nil
}
...
func (s *ContractService) LinkDocument(eid, cID, refType, refID, refNo string) (*model.ContractReference, *apperrors.AppError) {
	...
	if refType == "" {
		return nil, apperrors.NewValidationError("ref_type", "关联类型不能为空")
	}
	if refID == "" {
		return nil, apperrors.NewValidationError("ref_id", "关联ID不能为空")
	}
...
func (s *ContractService) ListDocuments(cID string) ([]model.ContractReference, *apperrors.AppError) {
	...
	if dbErr != nil {
		return nil, apperrors.ErrInternal.WithDetail("查询关联单据失败: " + dbErr.Error())
	}
	return refs, nil
}
...
func (s *ContractService) List(eid string, p, ps int, status string) ([]model.Contract, int64, *apperrors.AppError) {
	...
	if dbErr != nil {
		return nil, 0, apperrors.ErrInternal.WithDetail("查询合同列表失败: " + dbErr.Error())
	}
	return cs, total, nil
}
