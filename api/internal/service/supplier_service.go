...
func (s *SupplierService) Create(eid, name, contact, phone, email, addr string) (*model.Supplier, *apperrors.AppError) {
	...
	if name == "" {
		return nil, apperrors.NewValidationError("name", "供应商名称不能为空")
	}
	...
	if err := s.repo.Create(sup); err != nil {
		return nil, apperrors.ErrInternal.WithDetail("创建供应商失败: " + err.Error())
	}
	return sup, nil
}
...
func (s *SupplierService) Update(enterpriseID, supID, name, contact, phone, email, addr, status string) (*model.Supplier, *apperrors.AppError) {
	...
	id, err := uuid.Parse(supID)
	if err != nil {
		return nil, apperrors.NewValidationError("supplier_id", "供应商ID无效")
	}
	sup, err := s.repo.FindByID(id, eid)
	if err != nil {
		return nil, apperrors.ErrInternal.WithDetail("查询供应商失败")
	}
	if sup == nil {
	...
	if err := s.repo.Update(sup); err != nil {
		return nil, apperrors.ErrInternal.WithDetail("更新供应商失败: " + err.Error())
	}
	return sup, nil
}
...
func (s *SupplierService) Delete(enterpriseID, supID string) *apperrors.AppError {
	...
	id, err := uuid.Parse(supID)
	if err != nil {
		return apperrors.NewValidationError("supplier_id", "供应商ID无效")
	}
	sup, err := s.repo.FindByID(id, eid)
	if err != nil {
		return apperrors.ErrInternal.WithDetail("查询供应商失败")
	}
	if sup == nil {
		return apperrors.ErrNotFound.WithDetail("供应商不存在")
	}
	if err := s.repo.Delete(id, eid); err != nil {
		return apperrors.ErrInternal.WithDetail("删除供应商失败: " + err.Error())
	}
	return nil
}
...
func (s *SupplierService) List(eid string, p, ps int) ([]model.Supplier, int64, *apperrors.AppError) {
	...
	if err != nil {
		return nil, 0, apperrors.ErrInternal.WithDetail("查询供应商列表失败: " + err.Error())
	}
