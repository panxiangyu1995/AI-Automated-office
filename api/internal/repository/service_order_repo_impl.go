...
func (r *serviceOrderRepo) Delete(order *model.ServiceOrder, enterpriseID uuid.UUID) error {
	return r.db.Where("id = ? AND enterprise_id = ?", order.ID, enterpriseID).Delete(order).Error
}
...
func (r *serviceOrderRepo) List(enterpriseID uuid.UUID, orderType, status string, page, pageSize int) ([]model.ServiceOrder, int64, error) {
	...
	var total int64
	q := r.db.Model(&model.ServiceOrder{}).Where("enterprise_id=?", enterpriseID)
	if orderType != "" {
...
func (r *serviceOrderRepo) ListByCustomer(customerID, enterpriseID uuid.UUID) ([]model.ServiceOrder, error) {
	...
	if err := r.db.Where("customer_id = ? AND enterprise_id = ?", customerID, enterpriseID).Find(&orders).Error; err != nil {
		return nil, err
	}
	return orders, nil
}
...
func (r *serviceOrderRepo) UpdateStatus(id, enterpriseID uuid.UUID, status string) error {
	return r.db.Model(&model.ServiceOrder{}).Where("id = ? AND enterprise_id = ?", id, enterpriseID).Update("status", status).Error
}
