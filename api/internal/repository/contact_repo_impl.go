...
func (r *contactRepo) Delete(id, enterpriseID uuid.UUID) error {
	return r.fresh().Model(&model.Contact{}).Where("id = ? AND enterprise_id = ?", id, enterpriseID).UpdateColumn("deleted_at", time.Now()).Error
}
