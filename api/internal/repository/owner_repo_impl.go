...
func (r *ownerRepo) SumSalesOrderRevenue(enterpriseID uuid.UUID, statuses []string) (float64, error) {
	var amount float64
	err := r.db.Model(&model.SalesOrder{}).Where("enterprise_id=? AND status IN ?",
...
func (r *ownerRepo) SumCollections(enterpriseID uuid.UUID) (float64, error) {
	var amount float64
	err := r.db.Model(&model.CollectionRecord{}).Where("enterprise_id=?", enterpriseID).
...
func (r *ownerRepo) SumPayments(enterpriseID uuid.UUID) (float64, error) {
	var amount float64
	err := r.db.Model(&model.PaymentRecord{}).Where("enterprise_id=?", enterpriseID).
		Select("COALESCE(SUM(amount), 0)").Scan(&amount).Error
	return amount, err
}
...
func (r *ownerRepo) CountLowStock(enterpriseID uuid.UUID) (int64, error) {
	var count int64
	err := r.db.Model(&model.WarehouseInventory{}).Where("enterprise_id=? AND quantity <= safety_stock AND safety_stock > 0", enterpriseID).
		Count(&count).Error
	return count, err
}
...
func (r *ownerRepo) CountTotalSKU(enterpriseID uuid.UUID) (int64, error) {
	var count int64
	err := r.db.Model(&model.WarehouseInventory{}).Where("enterprise_id=?", enterpriseID).Count(&count).Error
	return count, err
}

func (r *ownerRepo) CountEmployeesByStatus(enterpriseID uuid.UUID, status string) (int64, error) {
	var count int64
	err := r.db.Model(&model.Employee{}).Where("enterprise_id=? AND status=?", enterpriseID, status).Count(&count).Error
	return count, err
}
...
func (r *ownerRepo) FindAlertRuleByID(id, enterpriseID uuid.UUID) (*model.AlertRule, error) {
	...
	if err := r.db.Where("id=? AND enterprise_id=?", id, enterpriseID).First(&rule).Error; err != nil {
...
func (r *ownerRepo) UpdateAlertRule(id, enterpriseID uuid.UUID, input map[string]interface{}) (*model.AlertRule, error) {
	...
	if err := r.db.Model(&rule).Updates(input).Error; err != nil {
		return nil, err
	}
	r.db.Where("id=? AND enterprise_id=?", id, enterpriseID).First(&rule)
	return &rule, nil
}

func (r *ownerRepo) ListAlertRules(enterpriseID uuid.UUID) ([]model.AlertRule, error) {
	var rules []model.AlertRule
	err := r.db.Where("enterprise_id=?", enterpriseID).Find(&rules).Error
	return rules, err
}
