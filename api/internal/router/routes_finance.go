...
func registerFinanceRoutes(protected *gin.RouterGroup, enterprise *gin.RouterGroup, deps *RouterDeps) {
	...
	protected.POST("/payment-requests/:id/submit", deps.PaymentRequestHandler.SubmitForApproval)
	protected.POST("/payment-requests/:id/approve", deps.PaymentRequestHandler.Approve)
	protected.POST("/payment-requests/:id/reject", deps.PaymentRequestHandler.Reject)
