...
本项目是**无前端 SaaS**，Agent 对话界面信息展示能力有限（画幅小、不支持复杂表格/图表）。因此，当返回数据量较大或需要可视化呈现时，Agent **必须**生成独立 HTML 文件供用户在浏览器中查看。
...
| 场景 | 生成 HTML？ | 原因 |
|------|------------|------|
| 列表数据 > 10 行 | **是** | 对话窗口无法有效展示长表格 |
| 仪表盘/KPI/经营数据 | **是** | 需要卡片式布局+颜色状态标识 |
| 审计日志/操作记录 | **是** | 需要时间线+筛选展示 |
| 多维度对比分析 | **是** | 需要图表（柱状图/饼图/趋势线） |
| 老板信号面板 | **是** | 红/黄/绿状态需要可视化 |
| 单条记录详情 | 否 | 对话窗口可展示 |
| 创建/更新/删除结果 | 否 | 一行确认即可 |
| 简单计数查询 | 否 | 直接文字回答 |
...
```
1. 通过 ao-cli skill execute 获取 JSON 数据
2. 解析 JSON，提取关键字段
3. 根据数据类型选择 HTML 模板（见 references/html-templates.md）
4. 用真实数据填充模板，生成完整 HTML 文件
5. 保存到用户指定目录（默认：项目根目录/test-flie/）
6. 告知用户文件路径，建议用浏览器打开
```
...
```
{报告类型}_{YYYYMMDD_HHmmss}.html

示例：
- dashboard_20260812_170000.html
- employee-list_20260812_170000.html
- audit-log_20260812_170000.html
- owner-kpi_20260812_170000.html
```
...
# Step 3: 告知用户
# "已生成经营驾驶舱报告，请用浏览器打开：test-flie/dashboard_20260812_170000.html"
```

详细的 HTML 模板规范和代码示例见 [references/html-templates.md](references/html-templates.md)。
