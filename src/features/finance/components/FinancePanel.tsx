...
import { Receipt, FileText, Wallet, TrendingUp, Loader2, RefreshCw } from 'lucide-react'
...
const INVOICE_STATUS_COLORS: Record<InvoiceStatus, string> = { pending: 'bg-yellow-500', verified: 'bg-green-500', recorded: 'bg-blue-500' }
const INVOICE_STATUS_LABELS: Record<InvoiceStatus, string> = { pending: '待审核', verified: '已验证', recorded: '已入账' }
const LEDGER_STATUS_COLORS: Record<LedgerStatus, string> = { pending: 'bg-yellow-500', partial: 'bg-blue-500', completed: 'bg-green-500' }
const LEDGER_TYPE_COLORS: Record<LedgerType, string> = { receivable: 'text-green-600', payable: 'text-red-600' }
...
export function FinancePanel() {
  const { invoices, ledgerEntries, stats, filterType, isLoading, fetchInvoices, fetchLedger, fetchStats, setFilterType } = useFinanceStore()
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Button variant="outline" size="sm" onClick={() => { fetchInvoices(); fetchLedger(filterType); fetchStats() }}>
      ...
      <Tabs defaultValue="invoices">
                ...
                <Card key={inv.id}>
                  <CardContent className="p-4 flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <Badge className={`${INVOICE_STATUS_COLORS[inv.status]} text-white border-0 mt-1`}>
        ...
        <TabsContent value="ledger">
          <div className="mb-4">
            <Select value={filterType || 'all'} onValueChange={(v) => setFilterType(v === 'all' ? undefined : v)}>
                ...
                <Card key={entry.id}>
                  <CardContent className="p-4 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-medium">{entry.ledgerType === 'receivable' ? '应收' : '应付'}</div>
                    ...
                    <div className="text-right">
                      ...
                      <Badge className={`${LEDGER_STATUS_COLORS[entry.status]} text-white border-0 mt-1`}>
                        {entry.status}
                      </Badge>
                    </div>
                  </CardContent>
