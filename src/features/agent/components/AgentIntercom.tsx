      ...
      <div className={cn('max-w-[70%] flex flex-col', isSent ? 'items-end' : 'items-start')}>
        ...
        <div className="flex items-center gap-2 mt-1 text-xs text-muted-foreground">
          ...
          {message.auditStatus !== 'approved' && (
            ...
            >
              {message.auditStatus === 'pending' ? '待审核' :
               message.auditStatus === 'rejected' ? '已拒绝' : '已标记'}
            </Badge>
          )}
        </div>
      </div>
        ...
        <div className="space-y-4 mt-4">
          <div className="flex items-center gap-3 p-3 rounded-lg bg-muted">
            ...
            <Badge className={cn('ml-auto', getSecurityLevelColor(contact.securityLevel))}>
              {contact.securityLevel === 'trusted' ? '可信' :
               contact.securityLevel === 'verified' ? '已验证' :
               contact.securityLevel === 'restricted' ? '受限' : '已阻止'}
            </Badge>
          </div>
  ...
  const stats: IntercomStats = useMemo(() => {
    ...
    return {
      ...
      sentToday: messages.filter(
        (m) => m.direction === 'sent' && m.timestamp >= today
      ).length,
      receivedToday: messages.filter(
        (m) => m.direction === 'received' && m.timestamp >= today
      ).length,
      pendingPermission: messages.filter(
        (m) => m.auditStatus === 'pending'
      ).length,
      blockedMessages: messages.filter(
        (m) => m.auditStatus === 'rejected'
      ).length,
      bySecurityLevel: {
  ...
  const handleSendMessage = () => {
    ...
    const newMessage: AgentMessage = {
      ...
      permissionDeniedReason:
        selectedContact?.permissionLevel === 'none'
          ? '通信被阻止'
          : undefined,
      ...
      prev.map((c) =>
        c.id === contact.id
          ? {
              ...c,
              securityLevel:
                c.securityLevel === 'blocked' ? 'verified' : 'blocked',
              permissionLevel:
                c.securityLevel === 'blocked' ? 'read' : 'none',
            }
          : c
      )
    )
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <Badge className={cn('text-xs', getSecurityLevelColor(selectedContact.securityLevel))}>
                      {selectedContact.securityLevel === 'trusted' ? '可信' :
                       selectedContact.securityLevel === 'verified' ? '已验证' :
                       selectedContact.securityLevel === 'restricted' ? '受限' : '已阻止'}
                    </Badge>
                  </div>
