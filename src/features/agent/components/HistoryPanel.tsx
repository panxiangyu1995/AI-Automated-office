...
export function HistoryPanel({ className, onSelectSession }: HistoryPanelProps) {
    ...
    if (filter.timeRange !== 'all') {
      ...
      result = result.filter((session) => {
        switch (filter.timeRange) {
          ...
          case 'yesterday':
            return session.updatedAt >= today.getTime() - 24 * 60 * 60 * 1000 && session.updatedAt < today.getTime()
          case 'week':
  ...
  const handleDeleteArchived = useCallback((archiveId: string) => {
      ...
      <div className="px-3 py-2 border-b border-slate-200">
        ...
        <div className="relative mt-2">
          <button
            ...
            style={filter.timeRange !== 'all' ? { borderColor: '#1E3A5F', color: '#1E3A5F' } : undefined}
          >
            <span className="flex items-center gap-2">
              <Clock size={14} />
              {TIME_FILTER_OPTIONS.find(o => o.value === filter.timeRange)?.label}
            </span>
            <ChevronDown size={14} className={cn('transition-transform', showTimeFilter && 'rotate-180')} />
          </button>
        ...
        {!showArchived ? (
          ...
          archivedSessions.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-32 text-slate-400">
              ...
              <p className="text-sm">暂无已归档的会话</p>
            </div>
          ) : (
            <div className="space-y-1">
