...
### 数据库命名规范
- 表名：snake_case 复数（如 `users`, `posts`）
- 列名：snake_case（如 `created_at`, `user_id`）
- 主键：`id`
- 外键：`{表名单数}_id`

### API 命名规范
- REST 端点：kebab-case 复数（如 `/api/invite-codes`）
- 路由参数：camelCase（如 `/api/posts/:postId`）
- 请求体字段：camelCase
...
1. **铁律优先** - PRD、架构、UX、Epic 文档是铁律，所有决策以此为基准
2. **匿名保护** - 系统不记录 IP 与账号的关联关系，这是核心设计原则
3. **合规检查** - 实现前必须完成铁律合规检查
4. **Test before commit** - 所有代码必须通过测试
5. **遵循命名约定** - 数据库、API、代码命名必须遵循规范
6. **统一响应格式** - API 必须返回统一的 `{code, data, message}` 格式
...
**记住：PRD + 架构 + UX + Epic = 铁律。违背铁律 = 项目失败。**