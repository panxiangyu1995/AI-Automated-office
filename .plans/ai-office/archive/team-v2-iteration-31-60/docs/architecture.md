...
- **Cloud Layer**: Go 云端后端 + 多租户 + 认证授权
  - **CSP 安全策略**（R2 新增）：tauri.conf.json 完整CSP，default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; connect-src self+localhost+openrouter.ai; frame-src 'none'
  - **RBAC 权限**（R2 新增）：Role 层级 (Employee < Manager < Admin) + Permission (Read/Write/Admin) + check_permission
