...
> **设计原则：** 用户主 Agent 及其 Sub-Agent 在某个部门上下文下只能调用当前权限范围内的部门内部工具，但可以调用其他部门暴露的协作工具。协作支持用户手动触发和 Agent 自动编排两种模式。
...
│                                                             │
│ 步骤2 [仓储部] 确认库存并发货                                 │
│        工具: warehouse_action (协作工具)                     │
│        参数: {action: "ship_order", entity: "shipment", data: {...}}│
│                                                             │
│ 步骤3 [财务部] 生成应收账款                                   │
│        工具: finance_mutate (协作工具)                       │
...
所有跨部门数据查询**统一通过云端 API 获取**，不走 P2P 直连或消息队列：
...
数据写入通过 **Tool Calling（工具调用）** 实现，分为人为操作和 AI 代理操作两种路径：
...
│  【人为操作路径】                                           │
│  ─────────────────                                          │
│  用户在 UI 操作 ──► 前端组件 ──► API 网关 ──► 业务服务 ──► 数据库│
│                                                              │
│  【AI 代理操作路径】                                        │
│  ─────────────────────                                     │
│  用户说"帮我..." ──► Agent 推理 ──► 工具调用 ──► API ──► 数据库│
│                           │                                 │
│                           ▼                                 │
│                      Tool Calling                            │
│                 tools.{dept}_{action}({...})                │
...
│    action: "create",                                        │
│    entity: "order",                                        │
│    data: { customer: "客户A", quantity: 100 }              │
│  })                                                        │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```

#### Tool Calling 是数据写入的桥梁

| 写入要素 | 说明 |
|---------|------|
| **工具定义** | 每个部门 Pilot 暴露一组工具（query/mutate/action/export） |
| **权限控制** | 工具调用前检查用户/Agent 是否有权限（FR470-FR471） |
| **审批流程** | 敏感操作需要审批（FR469 协作审批策略） |
| **审计日志** | 所有写入操作记录审计链路 |
| **事务保证** | 跨部门操作通过云端事务保证数据一致性 |

#### 消息机制的适用范围

| 用途 | 方式 | 说明 |
|------|------|------|
| **事件通知** | 消息队列 | 订单状态变更、审批结果等 |
| **异步任务** | 消息队列 | 批量处理、定时任务等 |
| **数据查询** | **不适用** | 必须通过云端 API 直查 |

> **注意**：消息机制用于事件驱动场景，**不是数据查询的途径**。数据查询必须通过 API 直连云端数据库，保证数据的实时性和一致性。
