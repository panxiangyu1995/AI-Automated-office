...
- **API配置三级体系**：平台官方API（出租方提供） → 租户级配置（管理员统一） → 用户级配置（员工自选）
- **本地部署支持**：支持客户自行搭建的本地LLM服务（OpenAI兼容格式）
...
| ADR-054 | LLM API配置采用三级体系：平台官方API（出租方提供） → 租户级配置（管理员统一） → 用户级配置（员工自选）；支持本地部署API接入，只要符合OpenAI兼容格式即可 | 多租户API配置澄清 2026-03-29 |
| ADR-055 | LLM Provider 采用 Plan/Act 双配置模式：Plan 模式使用高精度/低成本模型生成计划，Act 模式使用高性能模型执行任务；两种模式可独立配置 Provider、Model、API Key | cline研究借鉴 2026-03-29 |
...
> **背景说明：** 作为AI赋能的ERP系统，本系统需要支持灵活的多租户API配置模式。作为出租方，系统提供官方API服务；但租户可选择不使用官方API，而是自行配置第三方服务（DeepSeek、智谱AI等）或本地部署的API服务。同时，租户管理员可统一管理员工API配置，员工也可根据需要自行配置。
...
| 优先级 | 配置来源 | 说明 |
|-------|---------|------|
| 1（最高） | 用户级配置 | 员工个人API Key，优先于其他配置 |
| 2 | 租户级配置 | 租户管理员统一配置，覆盖平台官方 |
| 3（最低） | 平台官方配置 | 系统默认官方API，作为兜底 |
...
| 选项 | 说明 | 影响因素 |
|------|------|---------|
| `allow_user_api_config` | 是否允许员工自行配置API Key | 员工是否能看到个人API配置入口 |
| `allow_third_party_api` | 是否允许使用非官方第三方API | 是否显示第三方API配置选项 |
| `allow_local_deploy_api` | 是否允许使用本地部署API | 是否显示本地部署API配置选项 |
| `mandatory_tenant_config` | 是否强制使用租户统一配置 | 员工是否必须使用租户配置 |

### 支持的API提供方类型

| 类型 | 示例 | 配置要求 |
|------|------|---------|
| **平台官方API** | 出租方官方API服务 | 预置配置，无需用户输入Key |
| **第三方云服务** | DeepSeek、智谱AI、百炼、硅基流动等 | 需要用户提供API Key |
| **本地部署服务** | 本地LLM服务（Ollama等） | 需要用户提供API端点URL |

### Plan/Act 双配置模式

> **设计背景（参考 cline）：** Plan 模式用于生成任务计划（低频、高精度），Act 模式用于执行任务（高频、需要高吞吐）。两种模式对模型能力要求不同，应支持独立配置。
...
│  │ ───────────────────────────────────────────────────────────────────│   │
│  │ • 用途：生成任务计划、分解步骤、风险评估                             │   │
│  │ • 推荐模型：Claude Haiku、GPT-3.5-Turbo 等低成本模型                │   │
│  │ • 特点：高精度、低成本、可使用较长上下文                             │   │
│  │ • 工具限制：仅允许读取类工具（read、search），禁止写入/执行          │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                    │                                        │
│                                    ▼                                        │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
...
│  │ • 推荐模型：Claude Sonnet、GPT-4o、DeepSeek 等高性能模型            │   │
│  │ • 特点：高吞吐、支持工具调用、需要快速响应                           │   │
│  │ • 工具限制：允许所有工具，按敏感度评估审批                           │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ 配置示例                                                            │   │
...
### 路由模式（Yolo/Auto/Manual/Hybrid）

> **设计背景（参考 cline）：** 不同用户场景需要不同的自动化程度。Yolo 模式适合高级用户完全自动化，Manual 模式适合新手逐项审批。
...
> **设计背景（参考 cline）：** 配置值可能来自多个层级（远程配置、会话覆盖、任务设置、全局默认），需要明确的优先级规则和防抖持久化机制。
...
│                                    ▼                                        │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ Level 2: Session（会话覆盖）                                         │   │
│  │ ───────────────────────────────────────────────────────────────────│   │
│  │ • 当前会话特有的配置覆盖                                             │   │
│  │ • 如：CLI 传入的 --provider=openai 标志                              │   │
│  │ • 仅在当前会话生效                                                    │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                    │                                        │
│                                    ▼                                        │
...
│  │ Level 3: Task（任务设置）                                           │   │
│  │ ───────────────────────────────────────────────────────────────────│   │
│  │ • 当前任务的配置（如 Plan 模式专用配置）                             │   │
│  │ • 任务完成后恢复默认                                                 │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                    │                                        │
│                                    ▼                                        │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ Level 4: Global（全局默认） - 最低优先级                             │   │
│  │ ───────────────────────────────────────────────────────────────────│   │
│  │ • 用户全局默认配置                                                   │   │
│  │ • 配置文件默认值                                                     │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
...
> **设计背景（参考 cline）：** 频繁的配置修改不应立即触发磁盘写入，应合并为一次批量写入。
...
│  原理：                                                                    │
│  ─────────────────────────────────────────────────────────────────────────│
│  • 每次配置修改不立即写盘，而是加入待写入队列                              │
│  • 500ms 内的多次修改合并为一次写操作                                      │
│  • 减少 SSD/HDD 写入次数，延长磁盘寿命                                    │
│                                                                             │
│  实现流程：                                                                │
│  ─────────────────────────────────────────────────────────────────────────│
│  1. 配置修改 → 加入 pending_writes 队列                                    │
│  2. 重置 debounce_timer（500ms）                                          │
│  3. 500ms 内再有修改 → 重新计时                                            │
│  4. 500ms 无新修改 → 执行批量写入                                          │
│                                                                             │
│  代码示意：                                                                │
...
│      pending: Arc<RwLock<HashSet<String>>>,  // 待写入的 key               │
...
# Per-Tool Auto-Approve 配置（参考 cline）
auto_approve_tools = ["xianyu_get_orders", "xianyu_query.*"]  # 支持精确名或正则
...
command = "mcp-filesystem"
args = ["/path/to/workspace"]
# STDIO 文件监视自动重启（参考 cline）
watch_files = ["/path/to/workspace/build/index.js"]
restart_on_change = true
```

##### MCP Per-Tool Auto-Approve 策略（参考 cline）

> **设计背景：** 不同 MCP 工具风险等级不同，需要精细化配置审批策略。

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                      Per-Tool Auto-Approve 策略                            │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  配置方式：                                                                 │
│  ─────────────────────────────────────────────────────────────────────────│
│  auto_approve_tools:                                                       │
│    - "tool_name"           # 精确匹配                                      │
│    - "prefix_.*"           # 正则匹配（以 prefix_ 开头的所有工具）          │
│    - "^exact_name$"        # 正则精确匹配                                  │
│                                                                             │
│  示例配置：                                                                 │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ auto_approve_tools:                                                 │   │
│  │   - "read_file"          # 精确匹配：读取文件                        │   │
│  │   - "list_.*"            # 正则匹配：所有 list 开头的工具           │   │
│  │   - "query_.*"           # 正则匹配：所有查询类工具                  │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
│  审批流程：                                                                 │
│  ─────────────────────────────────────────────────────────────────────────│
│  1. 检查工具名是否在 auto_approve_tools 列表中（含正则匹配）                │
│  2. 匹配成功 → 自动执行，无需审批                                            │
│  3. 匹配失败 → 按路由模式（Manual/Auto/Hybrid）决定是否需要审批             │
│                                                                             │
│  安全建议：                                                                 │
│  ─────────────────────────────────────────────────────────────────────────│
│  • 高风险工具（delete、execute_command）建议保持需要审批                     │
│  • 读取类工具（read、list、search）可加入 auto_approve                     │
│  • 定期审计 auto_approve_tools 列表，移除不再需要的规则                     │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

##### MCP 传输类型与 OAuth 支持（参考 cline）

> **设计背景：** 企业级 MCP 服务需要支持多种传输协议和认证方式。
...
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────┐   │
│  │ SSE 传输（远程服务 - Server-Sent Events）                            │   │
│  │ ───────────────────────────────────────────────────────────────────│   │
│  │ • 适用于远程 MCP 服务器                                              │   │
│  │ • 支持 OAuth 认证                                                   │   │
│  │ • 单向实时推送                                                       │   │
...
##### MCP 文件监视自动重启（参考 cline）

> **设计背景：** STDIO 传输的 MCP 服务器在代码更新后需要手动重启才能生效。文件监视机制可以自动检测文件变化并触发重启。
...
│  触发条件：                                                                 │
│  ─────────────────────────────────────────────────────────────────────────│
│  • watch_files 列表中的文件发生变化                                        │
│  • 推荐监视 MCP 服务器的入口文件（如 build/index.js）                      │
│                                                                             │
│  配置参数：                                                                 │
│  ─────────────────────────────────────────────────────────────────────────│
│  watch_files: ["/path/to/server/build/index.js"]  # 监视路径列表          │
│  restart_on_change: true                            # 是否自动重启          │
│  restart_delay_ms: 1000                             # 重启延迟（防抖动）    │
│                                                                             │
│  工作流程：                                                                 │
│  ─────────────────────────────────────────────────────────────────────────│
│  1. MCP Hub 启动时建立文件监视（使用 notify crate）                        │
│  2. 文件变化事件触发 → 延迟 restart_delay_ms                               │
│  3. 延迟期间若无新变化 → 执行重启流程                                      │
│  4. 重启流程：断开连接 → 重启进程 → 重新握手                               │
│  5. 重连失败 → 标记为离线状态，记录错误日志                                │
│                                                                             │
│  使用场景：                                                                 │
│  ─────────────────────────────────────────────────────────────────────────│
│  • MCP 服务器正在开发中，代码更新后自动重载                                 │
│  • 配置文件变化后自动重载（无需重启整个应用）                               │
│  • 生产环境可禁用 restart_on_change，避免意外重启                           │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
